...
class Search extends React.Component {
    ...
    render() {
        const { width, select,selectChange, placeHolder, selectOptions, onClickSearch, onCancelSearch, className,CancelBtnShow,Value,onChange } = this.props;

        let CancelBtnDisplay =  CancelBtnShow?(CancelBtnShow==='y'?'block':'none'):(this.state.cancleShow === true ? 'block' : 'none');

        return (
            <div className={`search_container ${className ? className : ''}  ${this.state.SearchBlank?'border':''}`} style={{
                width: width ? width : '',
                borderColor: this.state.SearchBlank?'#ff0000':(this.state.inputFocus ? '#5897ed' : '#bac7d9'),
            }} >
...
class AppAlert extends React.Component {
    ...
    componentDidUpdate() {
        ...
        if (show) {
            ...
            if (type === 'success' || type === 'error' || type === 'tips'||type==='warn') {
      ...
      <React.Fragment>
        {show ? (
          maskShow ? (
            ...
            <div
              className={`alert_tips_tab ${show ? "animation" : ""} ${className?className:''}`}
              ref="alert_tips_tab"
            >
...

                        {
                                Menu && Menu.map((item, key) => {
                                    ...
                                    if (item.List) {
                                        ...
                                        return <div key={key} className={`frame_leftmenu_mainitem no_child ${pathname.includes(item.link)? "active selected" : ''}`}>

                                            <NavLink  onClick={e=>menuClick({id:item.id,name:item.name})} to={{pathname:item.link,state:{id:item.id,name:item.name}}}  className={`frame_leftmenu_mainitem_name ${item.menu ? item.menu : ''}`}>{item.name}</NavLink>
      ...
      // console.log(params)
        ...
        return (
            <AntdModal
                ...
                width={400}
                mask={this.state.mask}
                footer={type === 'examine'||type==='examineTeacher' ? [
                    ...
                    <div className='content-box'>
                        <div className='row'>
                            <span className='col-left'>
                                {type === 'student'|| type === 'graduate'||(type==='examine'&& this.state.role!=='teacher')? '学号': type === 'parents'?'编号': '工号'}
                            </span>
                            <span className='col-right' title={data.userID}>{data.userID ? data.userID : <span className='content-null'>未填写</span>}</span>
                        </div>
                        <div style={{ display: type=== 'parents' ? 'block' : 'none' }} className='row'>
                            <span className='col-left'>
                                {'子女学号'}
                            </span>
                            <span className='col-right' title={data.StudentID}>{data.StudentID ? data.StudentID : <span className='content-null'>未填写</span>}</span>
                        </div>
                        {/* <div style={{ display: type === 'parents' ? 'block' : 'none' }} className='row'>
                            <span className='col-left'>
                                {'子女姓名'}
                            </span>
                            <span className='col-right' title={data.StudentName}>{data.StudentName ? data.StudentName : <span className='content-null'>未填写</span>}</span>
                        </div> */}
                        ...
                        <div className='row' style={{ display: type !== 'leader'&&type !== 'graduate'&&type !== 'admin' ? 'block' : 'none' }}>
                            <span className='col-left'>
                                {type === 'student'||type==='examine'? '班级':type==='parents'?'子女班级' : '所教学科'}
                            </span>
                            <span className='col-right' title={type === 'student'||type==='examine'||type==='parents'?data.userGrade && data.userClass ? (data.userGrade + ' > ' + data.userClass) : '':data.subjectName ? data.subjectName:''}>
                                {type === 'student'||type==='examine'||type==='parents'? data.userGrade && data.userClass ? (data.userGrade + ' > ' + data.userClass) : <span className='content-null'>未填写</span> : data.subjectName ? data.subjectName : <span className='content-null'>未填写</span>}</span>
                        </div>
                        <div className='row' style={{ display: type === 'teacher'||type === 'examineTeacher' ? 'block' : 'none' }}>
                            <span className='col-left'>
                                {'职称'}
                            </span>
                        ...
                        {/* 用户档案 */}
                        <div className='row' style={{ display: this.state.module === 1 ? 'block' : 'none' ,marginTop: 20 + 'px' }}>
                            <span className='col-left'>
                                {'身份证号码'}
                            </span>
                            <span className='col-right'  title={data.userIDCard}>{data.userIDCard ? data.userIDCard : <span className='content-null'>未填写</span>}</span>
                        </div>
                        <div className='row' style={{ display: this.state.module === 1 ? 'block' : 'none' }}>
                            <span className='col-left'>
                                {'联系电话'}
                            </span>
                        ...
                        <div className='row row-adress' style={{ display: this.state.module === 1 ? 'block' : 'none' }}>
                            <span className='col-left'>
                                {'家庭住址'}
                            </span>
                        ...
                        {/* 用户账号 */}
                        <div className='row' style={{marginTop: 20 + 'px',  display: this.state.module === 2 ? 'block' : 'none' }}>
                            <span className='col-left' >
                                {'QQ'}
                            </span>
                            <span className='col-right'  title={data.QQ}>{data.QQ ? data.QQ : <span className='content-null'>未填写</span>}</span>
                        </div>
                        <div className='row' style={{ display:  this.state.module === 2 ? 'block' : 'none' }}>
                            <span className='col-left' >
                                {'微信'}
                            </span>
                        ...
                        <div className='row' style={{marginBottom: 20 + 'px', display:  this.state.module === 2 ? 'block' : 'none' }}>
                            <span className='col-left' >
                                {'联系电话'}
                            </span>
...
class Tips extends React.Component {
    ...
    render() {
        const { placement = 'right',getPopupContainer=(triggerNode) =>triggerNode.parentNode,children,overlayStyle, ...params } = this.props;
        // console.log(params)
        return (
            <Tooltip placement={placement} getPopupContainer={getPopupContainer} overlayStyle ={overlayStyle} className='Tooltips-red' {...params}>{children}</Tooltip>
        )
    }
}
