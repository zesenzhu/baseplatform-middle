...
class App extends Component {
          ...
          if (data) {
            if (hash.includes("Import")) {
              ...
              if(hash.includes("Student")){
                window.location.href = "/html/admclass#/Grade"//直接去到最新的版本
                return;
              }
              ...
              $('.frame-block-wrapper').css('padding-left','90px')
              dispatch({
    ...
    if (!stu) {
      ...
      if (grade) {
        let selectd = SchoolGradeClasses.Grades.find((item) => {
          return item.GradeID === gradeInfo.id;
        });
    ...
    if (AddClassModal.selectValue.value === 0) {
      ...
      if (AddClassModal.inputValue === "") {
        ...
        if (this.UserComm_CheckGroupName(AddClassModal.inputValue)) {
          ...
          //判断是否重名问题
          let selectGrade = SchoolGradeClasses.Grades.find((item) => {
            return item.GradeID === AddClassModal.selectValue.value;
          });
          //返回这个数组
          if (selectGrade) {
            //如果同名返回该同名数组index，如果不同名返回-1
            let classIndex = selectGrade.Classes.findIndex((item) => {
              return item.ClassName === AddClassModal.inputValue;
            });
            ...
            if (classIndex >= 0) {
              ...
              dispatch(
                UpDataState.addClass({
                  GradeID: AddClassModal.selectValue.value,
    ...
    switch (ident) {
      ...
      case "class":
        dispatch({
          ...
          info: { id: id, name: name, preName: preName, preId: preId },
        });
  ...
  Import(type) {
    if (type === 1) {
      window.open("/html/class/#/Import/Teacher");
    }
