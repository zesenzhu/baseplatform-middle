...
function ScheduleSetting(props) {
  ...
  const {
    MorningRadioChecked,
    MorningInputDisabled,
    MorningTime,

    AfternoonRadioChecked,
    AfternoonInputDisabled,
    AfternoonTime,
  } = AdjustClassHourModal;
    ...
    if (UserID) {
      ...
      GetTermInfoAndHolidayCount({ SchoolID, dispatch }).then((data) => {
        if (data) {
          ...
          const list =
            data.HolidayItem && data.HolidayItem.length > 0
              ? data.HolidayItem
              : [];
          ...
          if (getQueryVariable("isInitGuide")) {
            ...
            window.parent.postMessage(
              { module: "schedule", height: document.body.scrollHeight },
              `${protocol}//${host}`
            );
  ...
  //切换课表设置的方式
  const SettingTypeChange = (opts) => {
    const method = opts.type === 1 ? "分学段" : "统一";

    dispatch(
      AppAlertActions.alertQuery({
        title: `您确定要切换成按${method}设置吗？`,
        ok: () => {
          return (e) => SettingTypeSitch(opts);
        },
      })
    );
  };
  ...
  const MorningRadioChange = useCallback((e) => {
    dispatch({
      type: SSActions.MANAGER_SCHEDULE_SETTING_MORNING_RADIO_CHANGE,
      data: e.target.value,
    });
  }, []);
  ...
  const AfternoonRadioChange = useCallback((e) => {
    dispatch({
      type: SSActions.MANAGER_SCHEDULE_SETTING_AFTERNOON_RADIO_CHANGE,
      data: e.target.value,
    });
  }, []);
  ...
  const AdjustMorningInputChange = useCallback((e) => {
    ...
    dispatch({
      type: SSActions.MANAGER_SCHEDULE_SETTING_ADJUST_MORNING_INPUT_CHANGE,
      data: number,
    });
  }, []);
  ...
  const AdjustAfternoonInputChange = useCallback((e) => {
    ...
    dispatch({
      type: SSActions.MANAGER_SCHEDULE_SETTING_ADJUST_AFTERNOON_INPUT_CHANGE,
      data: number,
    });
  }, []);
  ...
  const AddClassHourHide = useCallback(() => {
    dispatch({
      type: SSActions.MANAGER_SCHEDULE_SETTING_ADD_CLASSHOUR_MODAL_HIDE,
    });
  }, []);
  ...
  const AddStartHourChange = useCallback((e) => {
    dispatch({
      type: SSActions.MANAGER_SCHEDULE_SETTING_ADD_CLASSHOUR_START_HOUR_CHANGE,
      data: e.target.value,
    });
  }, []);
  ...
  const AddStartMinChange = useCallback((e) => {
    dispatch({
      type: SSActions.MANAGER_SCHEDULE_SETTING_ADD_CLASSHOUR_START_MIN_CHANGE,
      data: e.target.value,
    });
  }, []);
  ...
  const AddEndHourChange = useCallback((e) => {
    dispatch({
      type: SSActions.MANAGER_SCHEDULE_SETTING_ADD_CLASSHOUR_END_HOUR_CHANGE,
      data: e.target.value,
    });
  }, []);
  ...
  const AddEndMinChange = useCallback((e) => {
    dispatch({
      type: SSActions.MANAGER_SCHEDULE_SETTING_ADD_CLASSHOUR_END_MIN_CHANGE,
      data: e.target.value,
    });
  }, []);
  ...
  const EditClassHour = useCallback((opts) => {
    ...
    dispatch({
      type: SSActions.MANAGER_SCHEDULE_SETTING_EDIT_CLASSHOUR_MODAL_SHOW,
      data: {
        ClassHourNO,
        Type,
  ...
  const EditClassHourHide = useCallback(() => {
    dispatch({
      type: SSActions.MANAGER_SCHEDULE_SETTING_EDIT_CLASSHOUR_MODAL_HIDE,
    });
  }, []);
  ...
  const EditStartHourChange = (e) => {
    ...
    dispatch({
      type: SSActions.MANAGER_SCHEDULE_SETTING_EDIT_CLASSHOUR_START_HOUR_CHANGE,
      data: number,
    });
  };
  ...
  const EditStartMinChange = (e) => {
    ...
    dispatch({
      type: SSActions.MANAGER_SCHEDULE_SETTING_EDIT_CLASSHOUR_START_MIN_CHANGE,
      data: number,
    });
  };
  ...
  const EditEndHourChange = (e) => {
    ...
    dispatch({
      type: SSActions.MANAGER_SCHEDULE_SETTING_EDIT_CLASSHOUR_END_HOUR_CHANGE,
      data: number,
    });
  };
  ...
  const EditEndMinChange = (e) => {
    ...
    dispatch({
      type: SSActions.MANAGER_SCHEDULE_SETTING_EDIT_CLASSHOUR_END_MIN_CHANGE,
      data: number,
    });
  };
  ...
  const DelClassHour = useCallback((opts) => {
    ...
    dispatch(
      AppAlertActions.alertQuery({
        title: "您确定要删除该课时么？",
        ok: () => {
          return () => DelClassHourOk({ PeriodID, ClassHourNO });
        },
      })
    );
  }, []);
  ...
  const DelClassHourOk = ({ PeriodID, ClassHourNO }) => {
    ...
    ApiActions.DeleteClassHourInfo({
      SchoolID,
      ClassHourNO,
      PeriodID,
      dispatch,
  ...
  return (
    <React.Fragment>
      <div className="schedule-setting-wrapper">
        ...
        <div className={"holiday-time-wrapper clearfix"}>
          <div className={"left-content"}>
            本学期自 <span className={"start-date date"}>{holiday.start}</span>{" "}
            起至 <span className={"start-date date"}>{holiday.end}</span> 止，共
            {holiday.allDays}天，其中节假日{holiday.holidays}天(含周末)。
          </div>

          <Button
            className={"set-holiday-btn"}
            type={"primary"}
        ...
        <div className="setting-content-wrapper">
          ...
          SettingByPeriod.PeriodSettingList.length > 1 ? (
            <div className="setting-type-wrapper">
              ...
              <div className="setting-type-switch clearfix">
                <div
                  className={`setting-type-tab left ${
                    SettingType === 1 ? "active" : ""
                  }`}
                  onClick={(e) => SettingTypeChange({ type: 1 })}
                >
                  分学段设置
                </div>

                <div
                  className={`setting-type-tab right ${
                    SettingType === 0 ? "active" : ""
                  }`}
                  onClick={(e) => SettingTypeChange({ type: 0 })}
                >
                  统一设置
                </div>
              </div>
      ...
      <Modal
        type={1}
        className="adjust-class-hour-modal"
        title="批量调整上课时间"
      ...
      >
        <div className="monring-setting setting-line clearfix">
          ...
          <RadioGroup value={MorningRadioChecked} onChange={MorningRadioChange}>
            <Radio type="gray" value="before">
              提前
            </Radio>

            <Radio type="gray" value="after">
              延后
            </Radio>
          </RadioGroup>
          ...
          <Input
            maxLength={2}
            min={1}
            disabled={MorningInputDisabled}
            value={MorningInputDisabled ? "" : MorningTime}
            onChange={AdjustMorningInputChange}
          />
        </div>
        ...
        <div className="afternoon-setting setting-line clearfix">
          ...
          <RadioGroup
            value={AfternoonRadioChecked}
            onChange={AfternoonRadioChange}
          >
            <Radio type="gray" value="before">
              提前
            </Radio>
          ...
          <Input
            maxLength={2}
            min={1}
            disabled={AfternoonInputDisabled}
            value={AfternoonInputDisabled ? "" : AfternoonTime}
            onChange={AdjustAfternoonInputChange}
          />
        </div>
      </Modal>
      ...
      <Modal
        type={1}
        className="add-class-hour-modal"
        title="新增上课节次"
      ...
      >
        <div className="start-time-wrapper">
          ...
          <Input
            onChange={AddStartHourChange}
            className="start-hour"
            maxLength={2}
            min={0}
            max={23}
            value={AddClassHourModal.StartHour}
          />

          <span className="unit">时</span>

          <Input
            onChange={AddStartMinChange}
            className="start-min"
            maxLength={2}
            min={0}
            max={59}
            value={AddClassHourModal.StartMin}
          />
        ...
        <div className="end-time-wrapper">
          ...
          <Input
            onChange={AddEndHourChange}
            className="end-hour"
            maxLength={2}
            min={0}
            max={23}
            value={AddClassHourModal.EndHour}
          />

          <span className="unit">时</span>

          <Input
            onChange={AddEndMinChange}
            className="end-min"
            maxLength={2}
            min={0}
            max={59}
            value={AddClassHourModal.EndMin}
          />
      ...
      <Modal
        type={1}
        className="edit-class-hour-modal"
        title="修改上课时间"
      ...
      >
        <div className="class-time-wrapper">
          ...
          <span className="class-hour-no">
            <span className="time">
              {EditClassHourModal.Type === 1 ? "上午" : "下午"}
              {EditClassHourModal.ClassHourName}
            </span>

            <span className="time-area">
              ({EditClassHourModal.OldStartTime}-{EditClassHourModal.OldEndTime}
              )
            </span>
          </span>
        </div>
        ...
        <div className="start-time-wrapper">
          ...
          <Input
            onChange={EditStartHourChange}
            className="start-hour"
            maxLength={2}
            min={0}
            max={23}
            value={EditClassHourModal.StartHour}
          />

          <span className="unit">时</span>

          <Input
            onChange={EditStartMinChange}
            className="start-min"
            maxLength={2}
            min={0}
            max={59}
            value={EditClassHourModal.StartMin}
          />
        ...
        <div className="end-time-wrapper">
          ...
          <Input
            onChange={EditEndHourChange}
            className="end-hour"
            maxLength={2}
            min={0}
            max={23}
            value={EditClassHourModal.EndHour}
          />

          <span className="unit">时</span>

          <Input
            onChange={EditEndMinChange}
            className="end-min"
            maxLength={2}
            min={0}
            max={59}
            value={EditClassHourModal.EndMin}
          />
