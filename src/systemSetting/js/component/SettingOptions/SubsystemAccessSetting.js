    ...
    if (status === 0) {
      ...
      dispatch(
        AppAlertAction.alertQuery({
          ...
          ok: () => {
            return this.SubsystemAccess.bind(this, SubSystemID, "open");
      ...
      dispatch(
        AppAlertAction.alertQuery({
          ...
          ok: () => {
            return this.SubsystemAccess.bind(this, SubSystemID, "close");
          }, //调用SubsystemAccess方法进行关闭
          okTitle: "是",
          cancelTitle: "否",
        })
      );
          ...
          <div key={key} className="subsystem-wall">
            ...
            <div className="subsystem-content">
              <div className="pic-bg">
                {" "}
                <img
                  src={item.SubSystemImgUrl}
                  alt="图片丢失"
                  title={item.SubSystemName}
                />
              </div>
              <span className="subsystemName" title={item.SubSystemName}>
                {item.SubSystemName}
              </span>
              <div className="user-type">
                {" "}
                <span title="可访问的用户类别"></span>
                {item.UserTypeString}
              </div>
              ...
              <div className="access-state">
                ...
                <button
                  className={`btn-state ${
                    item.SubSystemStatus === 1 && item.CanBeClose === true
                      ? "open"
                      : item.SubSystemStatus === 1 && item.CanBeClose === false
                      ? "ban"
                      : ""
                  }`}
                  onClick={(e) =>
        ...
        <div className="subsystem-access">
          ...
          <div className="access-status">
            ...
            <DropDown
              width={120}
              dropSelectd={{
                value: this.state.AccessDropValue,
                title: this.state.AccessDropTitle,
              }}
              dropList={[
                {
                  value: "2",
          ...
          <div className="users-type">
            ...
            <DropDown
              width={120}
              dropSelectd={{
                value: this.state.UserDropValue,
                title: this.state.UserDropTitle,
              }}
              dropList={[
                {
                  value: ``,
          ...
          >
            ...
            subsystemInfo.TotalClose !== 0 ? (
              <React.Fragment>
                ，其中<span>{subsystemInfo.TotalClose}</span>个已关闭访问
              </React.Fragment>
            ) : (
              ""
            )}
