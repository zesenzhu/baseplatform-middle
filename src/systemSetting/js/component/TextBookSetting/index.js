...
import { Modal, Loading, DropDown, Empty } from "../../../../common";
...
class TextBookSetting extends Component {
          ...
          dispatch(
            UpDataState.GetSubjectListData({
              ...
              func: (State) => {
                ...
                dispatch(
                  UpDataState.GetSubjectInfoData({
                    subjectId: Subject.SubjectID,
                  })
                );
                // });
  ...
  TextBookMsgModalOk = () => {
    ...
    const {
      ...
      DataState: {
        LoginUser: { SchoolID },
        TextBookData: { GradeSubjectList, NodeInfo },
      },
    } = this.props;
    this.setState(
      {
      ...
      () => {
        ...
        if (GradeSubjectList.length == 0) {
            ...
            UpDataState.GetGradeSubjectInfo({
              ...
              func: (State) => {
                ...
                for (let i = 0; i < State.GradeSubjectList.length; i++) {
                  console.log(
                    State.GradeSubjectList[i].GradeId,
                    this.state.TextBookData.GradeID
                  );
                  if (
                    State.GradeSubjectList[i].GradeId ==
                    this.state.TextBookData.GradeID
                    ...
                    for (
                      let j = 0;
                      j < State.GradeSubjectList[i].Subjects.length;
                      j++
                    ) {
                      if (
                        State.GradeSubjectList[i].Subjects[j].SubjectID ==
                        this.state.TextBookData.SubjectID
                      ) {
                ...
                dispatch(
                  UpDataState.SetNodeInfoData({ TextBookModalVisible: false })
                );
          ...
          for (let i = 0; i < GradeSubjectList.length; i++) {
            ...
            if (
              GradeSubjectList[i].GradeId == this.state.TextBookData.GradeID
            ) {
              dropListSelectidx1 = i;
              for (let j = 0; j < GradeSubjectList[i].Subjects.length; j++) {
                if (
                  GradeSubjectList[i].Subjects[j].SubjectID ==
                  this.state.TextBookData.SubjectID
                ) {
                  dropListSelectidx2 = j;
                  break;
            ...
            for (let i = 0; i < State.GradeSubjectList.length; i++) {
              ...
              if (State.GradeSubjectList[i].GradeId == GradeID) {
                ...
                for (
                  ...
                  if (
                    State.GradeSubjectList[i].Subjects[j].SubjectID == SubjectID
                  ) {
                    dropListSelectidx2 = j;
                    break;
                  }
  ...
  setTextBookOk = () => {
    ...
    if (this.state.showdata == 1) {
      ...
      let parem = {
        ...
        subjectId:
          GradeSubjectList[this.state.dropListSelectidx1].Subjects[
            this.state.dropListSelectidx2
          ].SubjectID,
      ...
      postData(url, parem, 2, "json")
        ...
        .then((json) => {
          ...
          if (json.StatusCode == 200) {
            ...
            dispatch(
              UpDataState.GetSubjectInfoData({
                subjectId: this.state.TextBookData.SubjectID,
              })
            );
          }
        });
    ...
    if (!Test.test(this.state.nodeValue)) {
      ...
      dispatch(
        AppAlertAction.alertError({
          type: "btn-tips",
          title:
            "目录名称格式不正确！目录名称只能由字母,数字,中文,空格，@#￥$&*（）()-+|、？《》<>[]【】组成",
        })
      );
      return;
    }
      ...
      <div className="TextBookSetting" id="TextBookSetting">
        ...
        <div className="TextBook-content">
          <Loading
            ...
            opacity={true}
            tip="请稍候..."
            style={{ opacity: "0.7", background: "#fff" }}
          >
            <Collapse
              activeKey={OpenList}
            ...
            >
              {SubjectList instanceof Array && SubjectList.length >  0 ? (
                ...
                <Empty type={'4'} title={"暂无学科"} style={{marginTop:'200px'}}></Empty>
              )}
            </Collapse>
          </Loading>
        </div>
        ...
        <Modal
          ...
          zIndex={2001}
        >
          {SetTextBookModalVisible ? (
            <SetTextBookModal
              setTextBookshow={this.setTextBookshow}
            ></SetTextBookModal>
          ) : (
            ""
          )}
          ...
          >
            ...
            >
              <div className="setTextBook-top">
                ...
                <div>
                  <span className="setTextBook-top-span">
                    <b>年级: </b>{" "}
                    {GradeSubjectList && GradeSubjectList.length > 0 ? (
                      <DropDown
                        width={150}
                        dropSelectd={
                          GradeSubjectList[this.state.dropListSelectidx1]
                        }
                        type="simple"
                        onChange={this.onDropChange1.bind(this)}
                        dropList={GradeSubjectList}
                        height={200}
                      ></DropDown>
                    ) : (
                      []
                  ...
                  <span className="setTextBook-top-span">
                    ...
                    {GradeSubjectList && GradeSubjectList.length > 0 ? (
                      <DropDown
                        ...
                        dropSelectd={
                          GradeSubjectList[this.state.dropListSelectidx1]
                            .Subjects[this.state.dropListSelectidx2]
                        }
                        type="simple"
                        onChange={this.onDropChange2.bind(this)}
                        dropList={
                          GradeSubjectList[this.state.dropListSelectidx1]
                            .Subjects
                        }
                  ...
                  <span className="setTextBook-top-span">
                    ...
                    {this.state.TermList ? (
                      <DropDown
                        width={100}
                        dropSelectd={
                          this.state.TermList[this.state.dropListSelectidx3]
                        }
                        type="simple"
                        onChange={this.onDropChange3.bind(this)}
                        dropList={this.state.TermList}
                        height={200}
                      ></DropDown>
                    ) : (
                      []
              ...
              <p className="setTextBook-content-p">教材目录：</p>
              <div className={`TextBook-tree `}>
                <Scrollbars style={{ width: 100 + "%", height: "100%" }}>
                  <div className="tree-list-div">
                    {this.state.chapters.length > 0
                      ? this.state.chapters.map((item, idx) => {
                          return (
                            <div
                              className="tree-list-parent tree-list-parent-div"
                              key={idx}
                            >
                              {this.state.TreeOnclicknode == idx ? (
                                <div className="setNodeName">
                                  <Input
                                    ...
                                    className="input-class"
                                    placeholder="请输入目录名称"
                                    onChange={this.changeNodeValue.bind(this)}
                                    maxLength={50}
                                ...
                                <div className="tree-list-parent">
                                  <div className="tree-list-parent-hover">
                                    {this.state.showNodeArr &&
                                    !this.state.showNodeArr.some((item1) => {
                                      return item1 == idx;
                                    }) ? (
                                      <i
                                        title="展开子目录"
                                    ...
                                    <span>
                                      ...
                                      {idx != this.state.chapters.length - 1 ? (
                                        <b
                                          onClick={() => {
                                            this.downNode(idx);
                                          }}
                                    ...
                                    <ul>
                                      {item.chapters && item.chapters.length > 0
                                        ? item.chapters.map((it, id) => {
                                            return (
                                              <li
                                                className="tree-list-parent"
                                                key={idx + "&" + id}
                                              >
                                                {this.state.TreeOnclicknode ==
                                                idx + "&" + id ? (
                                                  <div
                                                    className="setNodeName "
                                                    style={{
                                                      marginLeft: "65px",
                                                    }}
                                                  >
                                                    <Input
                                                      ...
                                                      className="input-class"
                                                      placeholder="请输入目录名称"
                                                      onChange={this.changeNodeValue.bind(
                                                        this
                                                      )}
                                                      maxLength={50}
                                                      value={
                                                        this.state.nodeValue
                                                      }
                                                      onFocus={() => {
                                                        this.inputFocus();
                                                      }}
                                                  ...
                                                  <div className="tree-list-parent tree-list-parent-li">
                                                    {" "}
                                                    <i></i> <i></i>
                                                    <b title={it.chapterName}>
                                                      {it.chapterName}
                                                    </b>
                                                    <span>
                                                      <b
                                                        onClick={() => {
                                                          this.renameChildNode(
                                                            idx,
                                                            id,
                                                            it.chapterName
                                                          );
                                      ...
                                      idx + "&-1" ? (
                                        ...
                                        <li>
                                          <i
                                            className="child-add-node"
                                            title="添加二级目录"
                                            onClick={() => {
                                              this.addChildNode(idx, -1);
                                            }}
                                          >
                                            +
                                          </i>
                                          <b className="child-add-node-b"></b>
                                        </li>
                                      )}
                  ...
                  {this.state.chapters.length == 0 &&
                  this.state.TreeOnclicknode != "-1" ? (
                    <div
                      className="initAddNode"
                      onClick={() => {
                        this.addNode(-1);
                      }}
                    >
                  ...
                  {this.state.TreeOnclicknode == "-1" ? (
                    <div className="setNodeName" style={{ marginLeft: "28px" }}>
                      <Input
                        ref="myinput"
                        className="input-class"
                        placeholder="请输入目录名称"
                      ...
                      <b
                        onClick={() => {
                          this.setState({ TreeOnclicknode: -2 });
                        }}
                      >
                        取消
                      </b>{" "}
