...
function Account(props) {
  ...
  const { UserID, UserType, SchoolID } = useSelector(
  ...
  useEffect(() => {
    ...
    GetUserIdentify({ UserID }).then((data) => {
      if (data && data.length > 0) {
        ...
        data.map((i, k) => {
          identifyStr =
            k > 0 ? identifyStr + `、${i.IdentityName}` : i.IdentityName;
        });
        ...
        btnQueryAlertShow({
          title: (
            <div className={"account-title"}>
              确定重置
              <span className="user-name red" title={userArchives.UserName}>
                {userArchives.UserName}
              </span>
              <span className={"gray user-id"}>({UserID})</span>的密码?
            </div>
          ),

          abstract: (
            <div className={"new-pwd gray"}>
              <div className={"pwd-wrapper"}>
                新密码:
                <Tips visible={pwd.tip} title={pwd.title}>
                  <Input
                    className={"new-pwd-input"}
                    value={pwd.value}
                    onBlur={pwdBlur}
                    onChange={pwdChange}
                  ></Input>
              ...
              <div
                ...
                style={{ display: `${pwd.strongType ? "block" : "none"}` }}
              >
                密码强度:<i className={`strong-type ${pwd.strongType}`}></i>
                {pwd.strongTypeTxt}
              </div>
            </div>
          ),
      ...
      dispatch(
        btnQueryAlertShow({
          title: (
            <div className={"account-title"}>
              确定重置
              <span className="user-name red">{userArchives.UserName}</span>
              <span className={"gray user-id"}>({UserID})</span>的密码?
            </div>
          ),
          ...
          abstract: (
            <div className={"new-pwd gray"}>
              ...
              <div
                className={"strong-wrapper"}
                style={{ display: `${strongType ? "block" : "none"}` }}
              >
                密码强度:<i className={`strong-type ${strongType}`}></i>
                {strongTypeTxt}
              </div>
            </div>
          ),
  ...
  const updatePwdOk = useCallback(() => {
    ...
    if (!tip) {
      ResetPwd({ userID: UserID, userType: UserType, newPwd, dispatch }).then(
        (data) => {
          if (data === 0) {
  ...
  const logDataSource = useMemo(() => {
    if (userArchives.LoginLogList && userArchives.LoginLogList.length > 0) {
      return userArchives.LoginLogList.filter((i, k) => k <= 9).map((i) => ({
        ...i,
        key: i.LogID,
      }));
    } else {
          ...
          {[
            ...
            <tbody>
              <tr>
                ...
                <td className={"col2"}>
                  <div className={"short-name"}>
                    {isHasValue(userArchives.ShortName)}
                  </div>
                </td>
                ...
                <td className={"col4"}>
                  <div className={"last-log"}>
                    <span className={"time"}>
                      {isHasValue(userArchives.LastTimeLogin)}
                    </span>

                    {userArchives.LoginLogList.length > 0 ? (
                      <button onClick={moreLogShow} className={"more"}>
                        更多
                      </button>
                    ) : null}
                  </div>
              ...
              <tr>
                ...
                <td className={"col2"}>
                  {isHasValue(userArchives.SignUpTime)}
                </td>
                <td className={"col3 props"}>累计在线:</td>
                <td className={"col4"}>
                  {isHasValue(userArchives.LoginTimeSpan_Txt)}
                </td>
              </tr>
                ...
                <td className={"col4"}>
                  {userArchives.GangerName ? (
                    <button
                      ...
                      className={"ganger"}
                    >
                      {isHasValue(userArchives.GangerName)}
                    </button>
                  ) : (
                    "--"
                  )}
            ...
            <tbody>
              ...
              <tr>
                ...
                <td className={"col4"}>
                  {userStatus ? isHasValue(userStatus.nation) : "--"}
                </td>
              </tr>
              ...
              <tr>
                ...
                <td className={"col2"}>
                  {userStatus ? isHasValue(userStatus.professionalTitle) : "--"}
                </td>
