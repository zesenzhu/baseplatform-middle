...
const getAllUserPreview = (url) => {
  return (dispatch) => {
    ...
    getData(CONFIG.UserAccountProxy + url, 2)
      .then((res) => {
        return res.json();
      })
      .then((json) => {
...
//获取学生账号信息/改
const GET_PARENTS_PREVIEW = "GET_PARENTS_PREVIEW";
const getParentsPreview = (url) => {
  let pageIndex = url.split("PageIndex");
  let pageSize = url.split("PageSize");
...
const GetConfig = ({ schoolID, func = () => {} }) => {
  return (dispatch, getState) => {
    ...
    getData(
      CONFIG.UserAccountProxy + "/GetConfig?SchoolID=" + schoolID,
      ...
      .then((json) => {
        if (json.StatusCode === 200) {
          dispatch({ type: GET_CONFIG, data: json.Data });
          func(getState());
        }
      });
  };
